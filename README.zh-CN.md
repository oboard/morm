...
| `#morm.id(strategy="...")` | `#morm.id(strategy="uuid")` | 声明主键生成策略，当前会写入列 engine option：`pk.strategy=<value>`；支持 `manual` / `auto_increment` / `uuid` |
...
```text
SELECT * FROM student WHERE id = ? AND name = ? ORDER BY id DESC LIMIT 5 OFFSET 5
```
